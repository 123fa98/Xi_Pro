...
local Themes = {
	...
	Monochrome = {
		...
		DropdownBorder = Color3.fromRGB(200, 200, 200),
		DropdownOption = Color3.fromRGB(140, 140, 140),
		Keybind = Color3.fromRGB(120, 120, 120),
...
Creator.Theme = Creator.Theme or "Monochrome"
...
Components.Element = (function()
	...
	return function(Title, Desc, Parent, Hover, Options)
		...
		Element.Frame = New("TextButton", {
			Visible = Options.Visible 且 Options.Visible 为真，
		...
		函数 Element:SetTitle(Set)
			Element.TitleLabel.Text = 设置
		结尾

		函数 Element:Visible(Bool)
			Element.Frame.Visible = Bool
		结尾

		函数 Element:SetDesc(Set)
			如果 Set == nil 则
				设置 = ""
			结尾
			如果 Set == "" 则
			...
			结尾
			Element.DescLabel.Text = 设置
		结尾

		函数 Element:GetTitle()
			返回 Element.TitleLabel.Text
		结尾

		函数 Element:GetDesc()
			返回 Element.DescLabel.Text
		结尾
		...
		如果悬停
			...
			Creator.AddSignal(Element.Frame.MouseEnter, function()
				设置透明度(Creator.GetThemeProperty("ElementTransparency") - Creator.GetThemeProperty("HoverChange"))
			结尾）
			Creator.AddSignal(Element.Frame.MouseLeave, function()
				设置透明度(Creator.GetThemeProperty("ElementTransparency"))
			结尾）
	...
	function SaveManager:BuildConfigSection(tab)
		...
		AutoloadButton = section:AddButton({Title = "Set as autoload", Description = "Current autoload config: none", Callback = function()
			...
			self.Library:Notify({
				标题 = "界面",
				内容 = "配置加载器",
				SubContent = string.format("设置 %q 为自动加载", name),
				时长 = 7
			})
		结尾}）

		如果 isfile(self.Folder .. "/autoload.txt") 则
			local name = readfile(self.Folder .. "/autoload.txt")
			AutoloadButton:SetDesc("当前自动加载配置：" .. name)
		结尾
...
local InterfaceManager = {} do
	...
	函数 InterfaceManager:SetTheme(name)
		InterfaceManager.Settings.Theme = 名称
	结尾

	函数 InterfaceManager:SetFolder(folder)
		self.Folder = folder;
		self:BuildFolderTree()
	结尾

	函数 InterfaceManager:SetLibrary(library)
		self.Library = library
	结尾

	函数 InterfaceManager:BuildFolderTree()
		本地路径 = {}

		本地部分 = self.Folder:split("/")
		for idx = 1, #parts do
	...
	函数 InterfaceManager:SaveSettings()
		writefile(self.Folder .. "/options.json", httpService:JSONEncode(InterfaceManager.Settings))
	结尾

	函数 InterfaceManager:LoadSettings()
		local path = self.Folder .. "/options.json"
		如果 isfile(path)
			本地数据 = readfile(路径)
			本地成功，解码 = pcall(httpService.JSONDecode, httpService, data)
	...
	函数 InterfaceManager:BuildInterfaceSection(tab)
		assert(self.Library, "必须设置 InterfaceManager.Library")
		本地库 = self.Library
		本地设置 = InterfaceManager.Settings

		界面管理器：LoadSettings()

		本地部分 = tab:AddSection("界面")
		local InterfaceTheme = section:AddDropdown("InterfaceTheme", {
			标题 = "主题",
			描述 = “更改界面主题。”
			值 = Library.Themes，
			默认值 = self.Library.Theme，
			回调函数 = 函数(值)
...
local MinimizeButton = New("TextButton", {
	...
	New("ImageLabel", {
		Image = Mobile and "rbxassetid://82032993675869" or "",
		Size = UDim2.new(1, 0, 1, 0),
		BackgroundTransparency = 1,
	}, {
